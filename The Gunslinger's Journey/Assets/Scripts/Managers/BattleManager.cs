    ...
    void Start()
    {
        ...
        foreach (GameObject foe in allEnemies)
        {
            ...
            while (!turnPositionFound)
            {
                ...
                int thisCharacterSpeed = thisCharacterScript.GetEvasionSkill();
                int enemySpeed = enemyScript.GetEvasionSkill();
        ...
        if (introTextSeen && !endConditionsMet)
        {
            ...
            if (playerScript.GetTurn() && action == playerAction.Attack)
            {
                ...
                else if (timer >= buffer)
                {
                    if (!gotDamage)
                    {
                        playerScript.Attack(out damage);
                        gotDamage = true;
                    }
